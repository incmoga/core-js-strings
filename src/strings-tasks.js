...
function countVowels(str) {
  ...
  return str.split('').filter((char) => vowelsSet.has(char)).length;
}
...
function isPalindrome(str) {
  const strClean = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  const strRevers = strClean.split('').reverse().join('');
  return strClean === strRevers;
}
